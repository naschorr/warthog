...
def create_bar_squad_tier_distribution(global_performance_df: pd.DataFrame, *, player_name=None, country_filters=[]):
    """
    Create an interactive Plotly stacked bar chart showing battle tier rating percentages by squad type.

    Args:
        global_performance_df: DataFrame with performance data including tier status (should include squad, team, auto_squad columns)
    ...
    for tier_status in reversed(PLOTLY_BATTLE_RATING_TIER_STATUS_ORDER):
        ...
        percentages = [data["tier_percentages"][tier_status]["percentage"] for data in squad_tier_data]
        counts = [data["tier_percentages"][tier_status]["count"] for data in squad_tier_data]
