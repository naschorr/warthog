...
def create_heatmap_score_by_country_and_br(global_performance_df: pd.DataFrame, *, country_filters=[]):
    """
    ...
    heatmap_data = (
        df.groupby(["player.country", "player.battle_rating"])["player.score"].agg(["mean", "count"]).reset_index()
    )
    ...
    score_pivot = heatmap_data.pivot(index="player.country", columns="player.battle_rating", values="mean")
    ...
    title_filters["Total Players"] = len(df)
    title_filters["Unique Players"] = df["player.username"].nunique()
    if country_filters:
